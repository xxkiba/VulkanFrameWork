...
namespace FF {
        ...
        bool enableDynamicViewPort)
    {
        ...
        VkViewport viewport{};
		if (needFlipVewport) {
			// If flip viewport is needed, we flip the y-axis
			viewport.x = 0.0f;
			...
			// Normal viewport without flipping
			viewport.x = 0.0f;
			viewport.y = 0.0f;
			viewport.width = static_cast<float>(mWidth);
			viewport.height = static_cast<float>(mHeight);
		...
		if (enableDynamicViewPort) {
			mPipeline->mDynamicState.dynamicStateCount = 2; // Viewport and Scissor
			mPipeline->dynamicStates = { VK_DYNAMIC_STATE_VIEWPORT, VK_DYNAMIC_STATE_SCISSOR };
			mPipeline->mDynamicState.pDynamicStates = mPipeline->dynamicStates.data();
		}
		else {
    ...
    void OffscreenPipeline::buildScreenQuadPipeline(const Wrapper::RenderPass::Ptr& renderPass,
        uint32_t width, uint32_t height,
        const std::string& vertexShaderFile, const std::string& fragShaderFile,
        const std::vector<VkDescriptorSetLayout>& descriptorSetLayouts,
        const std::vector<VkPushConstantRange>* pushConstantRanges,
